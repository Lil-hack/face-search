...
if __name__ == '__main__':
    ...
    detector = MTCNN()
    i=0
    for list_photos in os.walk('/home/zikboy/Documents/data/men2'):
            for photo in list_photos[2]:
                i=i+1
                if i>0:
                    img = cv2.imread('/home/zikboy/Documents/data/men2/{}'.format(photo), cv2.COLOR_BGR2GRAY)
                    cropped = crop_face(img, detector)
                    # print (img)
                    # print('')
                    try:
                        norm_face=cv2.resize(cropped, (200, 200), interpolation=cv2.INTER_LINEAR)
                        # norm_face=cv2.normalize(norm_face, norm_face, 0, 255, cv2.NORM_MINMAX)

                        cv2.imwrite(r'{}.jpg'.format(uuid.uuid1()), norm_face)
                        Thread.sleep(5)
                    except:
                        pass
